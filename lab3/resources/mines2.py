...
def new_game(num_rows, num_cols, bombs):
    mask = make_board(num_rows, num_cols, False) # never used
    ...
    # mask should be mask not board

def dig(game, row, col): # maybe closure??
    ...
    squares_revealed_initial = revealed_count(game) # recursion ?
    do_dig(game,row,col)
