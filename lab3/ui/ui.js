...
// Changes
// do not save game state
// do not save squares dug
// no xray reveal on win/lose
// save which size is selected and load a new game of that size on select
// give loader spinner some padding width to prevent scrollbar flickering
// hide crash report before startup
...
function handle_xray_button() {
  if (!xray_state) {
    document.getElementById('xray_button').innerHTML = "XRAY ON (GAME PAUSED)";
    xray_state = true;
  }
  else {
...
function handle_renderer_button() {
  ...
  var text = our_renderer? "USING WORKING RENDERER": "USING MODULE'S RENDERER";
  document.getElementById('renderer_button').innerHTML = text;
